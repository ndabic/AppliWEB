...
function toggleShow(popupInfo) {
	exitButton = "toggleShow('exit')";
	switch (popupInfo) {
	case 'salle':
		
		popUp.innerHTML = '<div class="input-line">'+
						'<h2 class="title-'+popupInfo+'">&#160;'+popupInfo+'&#160;</h2>'+
						'<button onclick="'+exitButton+'" class="add-button button-exit">&#160;X&#160;</button>'+
						'</div>'+
						'<form name="addSalle" method="post" action="Serv" onsubmit="return validateForm()">'+
							'<div class="input-line">'+
							'<label for="nom-salle">Nom:</label>'+
				            '<input type="text" id="nom-salle" name="nom-salle"><br>'+
				            '</div>'+
				            '<input type="submit" value="Connexion" name="loginButton">'+
